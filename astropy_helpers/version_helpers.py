...
extension_helpers_version = "{ahver}"
"""[1:]
...
def _get_version_py_str(packagename, version, githash, release, debug,
                        uses_git=True):
    try:
        from extension_helpers import __version__ as ahver
    except ImportError:
        ahver = "unknown"
    ...
    if uses_git:
        ...
        # See https://github.com/astropy/extension-helpers/issues/141
...
def _generate_git_header(packagename, version, githash):
    ...
    if not source_lines:
        log.warn('Cannot get source code for extension_helpers.git_helpers; '
...
def get_pkg_version_module(packagename, fromlist=None):
    ...
    `extension_helpers.version_helpers.generate_version_py`.  Raises an
