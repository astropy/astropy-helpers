...
# _extension_helpers_test_--this will prevent any confusion by pkg_resources with
# any already installed packages named extension_helpers
# We also disable auto-upgrade by default
ah_bootstrap.DIST_NAME = 'extension-helpers-test'
ah_bootstrap.PACKAGE_NAME = '_extension_helpers_test_'
...
try:
    ...
    ah_bootstrap.use_extension_helpers({args})
finally:
    ah_bootstrap.DIST_NAME = 'extension-helpers'
    ah_bootstrap.PACKAGE_NAME = 'extension_helpers'
...
import _extension_helpers_test_
filename = os.path.abspath(_extension_helpers_test_.__file__)
...
def test_bootstrap_from_submodule(tmpdir, testpackage, capsys):
    """
    Tests importing _extension_helpers_test_ from a submodule in a git
    repository.  This tests actually performing a fresh clone of the repository
    without the submodule initialized, and that importing extension_helpers in
    that context works transparently after calling
    `ah_boostrap.use_extension_helpers`.
    """
    ...
    with orig_repo.as_cwd():
        ...
        # any previous reference to extension_helpers is first wiped from
        # sys.modules
        orig_repo.join('setup.py').write(TEST_SETUP_PY.format(args='', extra=''))
        run_cmd('git', ['add', 'setup.py'])

        # Add our own clone of the extension_helpers repo as a submodule named
        # extension_helpers
        run_cmd('git', ['submodule', 'add', str(testpackage),
                        '_extension_helpers_test_'])
        ...
        # Ensure that the extension_helpers used by the setup.py is the one that
        # was imported from git submodule
        a = os.path.normcase(path)
        b = os.path.normcase(str(tmpdir.join('clone', '_extension_helpers_test_',
                                             '_extension_helpers_test_',
                                             '__init__.py')))
        assert a == b
...
def test_check_submodule_no_git(capsys, tmpdir, testpackage):
    """
    Tests that when importing extension_helpers from a submodule, it is still
    ...
    with orig_repo.as_cwd():
        ...
        if 'UpgradeError' in stderr:
            pytest.fail('Attempted to run auto-upgrade despite importing '
                        '_extension_helpers_test_ from a git submodule')
...
def test_bootstrap_from_directory(tmpdir, testpackage, capsys):
    """
    Tests simply bundling a copy of the extension_helpers source code in its
    entirety bundled directly in the source package and not in an archive.
    """

    source = tmpdir.mkdir('source')
    testpackage.copy(source.join('_extension_helpers_test_'))
    ...
    with source.as_cwd():
        ...
        # was imported from git submodule
        a = os.path.normcase(path)
        b = os.path.normcase(str(source.join('_extension_helpers_test_',
                                             '_extension_helpers_test_',
                                             '__init__.py')))
        assert a == b
...
def test_bootstrap_from_archive(tmpdir, testpackage, capsys):
    """
    Tests importing _extension_helpers_test_ from a .tar.gz source archive
    ...
    with orig_repo.as_cwd():
        ...
        # directory that the _extension_helpers_test_ package was imported from
        ...
        a = os.path.normcase(path)
        b = os.path.normcase(str(egg.join('_extension_helpers_test_',
                                          '__init__.py')))
...
def test_download_if_needed(tmpdir, testpackage, capsys):
    """
    Tests the case where extension_helpers was not actually included in a
    ...
    with source.as_cwd():
        ...
        # easy_install should have worked by 'installing' extension_helpers as a
...
def test_upgrade(tmpdir, capsys):
    ...
    # Make a test package that uses _extension_helpers_test_
    source = tmpdir.mkdir('source')
    dist_dir = source.mkdir('dists')
    orig_dir.copy(source.join('_extension_helpers_test_'))
    ...
    # Make additional "upgrade" versions of the _extension_helpers_test_
    ...
    with source.as_cwd():
        ...
        # *version* of _extension_helpers_test_ was used
        run_setup('setup.py', [])
        ...
        b = os.path.normcase(str(egg.join('_extension_helpers_test_',
                                          '__init__.py')))
        assert a == b
        assert 'extension_helpers_test-0.1.1-' in str(egg)
