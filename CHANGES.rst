...
- Avoid importing whole package when trying to get version information. Note
  ...
  generated by extension-helpers. Instead, you can import these from
...
- Added extensive documentation about extension-helpers to the README.rst file. [#416]
...
- Remove all sphinx components from ``extension-helpers``. These are now replaced
...
- Fix segmentation faults that occurred when the extension-helpers submodule
...
- Make sure that extension-helpers 3.x.x is not downloaded on Python 2. [#362, #363]
...
- Add --auto-use and --no-auto-use command-line flags to match the
  ...
  ``--use-system-extension-helpers`` for ``--no-auto-use``. [#366]
...
- The 'automodapi' Sphinx extension (and associated dependencies) has now
  ...
  extension-helpers under extension_helpers.extern.automodapi for
  convenience. Version shipped with extension-helpers is v0.6.
  [#278, #303, #309, #323]
...
- Force the use of Cython's old ``build_ext`` command. A new ``build_ext``
  command was added in Cython 0.25, but it does not work with extension-helpers
  currently.  [#261]
...
- Various fixes enabling the extension-helpers Sphinx build command and
  Sphinx extensions to work with Sphinx 1.3. [#148]

- More improvement to the ability to handle multiple versions of
  extension-helpers being imported in the same Python interpreter session
...
- Improved the ``ah_bootstrap`` script's ability to override existing
  installations of extension-helpers with new versions in the context of
  ...
  package that uses a different version of extension-helpers. [#144]
...
- Fixed a bug that could occur when running ``setup.py`` for the first time
  in a repository that uses extension-helpers as a submodule:
...
- Initial release of extension-helpers.  See `APE4
